...
Benchmark: measure seek-to-first-frame time across the four trace fixtures. Record results. Regressions block merges. **Shipped:** `PlannerSeekBenchTests.swift` (XCTest `measure(metrics: [XCTClockMetric()])`, 4 fixtures, fresh planner+session per iteration); `PlannerSeekBenchRecorder.swift` (opt-in N=20 recorder gated on `BUTTERBAR_RECORD_SEEK_BASELINE=1`); `scripts/run-seek-bench.sh`; `docs/benchmarks/README.md`; `docs/benchmarks/seek-baseline.json` (arm64 / macOS 26.5: p50 0.039–0.058 ms). Spec gap (specs 02/04/05 name no numerical SLA) captured in follow-up issue #107 tagged `[opus]`. Opus APPROVE; 3 cosmetic fixups applied in-PR. Closes #105. Follow-ups: ~~#107 (SLA + regression threshold): planner bench is a regression guard; `regression_threshold_pct = 50.0` uniform per fixture; advisory-only CI; E2E SLA deferred to v1.5+~~ — resolved by addendum A25 (2026-04-16).

### T-DOC-ARCHITECTURE `[opus]` · DONE 2026-04-16
`docs/architecture.md` now describes the shipped v1 for future contributors: app/XPC/engine process boundary, streaming path, alert/event path, persistence model, CI/libtorrent dependency layout, and rejected implementation paths (`createTestTorrent`, `hash_failed_alert`-driven eviction, versioned Cellar paths). Also records that #33/#34/#36 remain product-surface design work per spec 07, not mechanical engine hardening.
