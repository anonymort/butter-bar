...
2. **Spec 06 (Brand) bumped to rev 2** with Tahoe-specific changes:
   - Logo concept rewritten to match the supplied package: butter pat resting on bar/shelf with a carved play symbol. Three ideas (butter, bar, play) in a single mark.
   - Asset specifications rewritten around the supplied `icons/` folder (`butter-bar-logo.svg`, `butter-bar-logo-1024.png`, `@1x/@2x/@3x` raster exports, `ButterBar.iconset/`, `ButterBar.icns`).
   - Icon workflow migrated from `AppIcon.appiconset/` (raster sizes) to `AppIcon.icon` (Apple's new Icon Composer format introduced in Tahoe). The legacy `.iconset/` and `.icns` are retained as a fallback path but inactive at the v1 deployment target.
   - Squircle compliance section added — Tahoe enforces squircle masking for app icons, and the supplied master complies.
   - Liquid Glass section added — glass is for floating navigation chrome only (toolbar, sidebar, HUD); never on content (library rows, settings rows, detail panels).
   - Window chrome notes updated to reflect automatic Tahoe glass treatment of toolbars and sidebars.

3. **Spec 02 (StreamHealth)** unchanged in content, but the UI rendering contract's pointer to "06-brand.md § Tier colours" still resolves correctly — the section name is preserved across the rev 2 rewrite.

4. **`docs/logo-generation-prompt.md` removed.** The Gemini Nano Banana / Image-3 prompt is no longer needed because the logo package has been supplied as a complete deliverable. References to it in `docs/claude-code-setup.md` have been removed.

5. **`T-BRAND-ASSETS` task in `TASKS.md`** rewritten:
   - No longer requires generating the artwork — the supplied `design/icons/` folder is the source.
   - Now requires running the supplied `butter-bar-logo-1024.png` through Apple's Icon Composer to produce `App/Assets.xcassets/AppIcon.icon`.
   - Acceptance now includes a squircle-compliance check (no grey squircle jail border in Finder/Dock).

6. **Reading order:** spec 09 added as item 9 in the reading order; specs 07 and 08 shift to items 10 and 11; TASKS.md is item 12; agents are item 13. Spec 09 is mandatory reading for any agent touching deployment configuration, Info.plist, or icon assets.

**Cascading deletions:**
- `docs/logo-generation-prompt.md` — removed.
- All references to "Big Sur+ squircle mask" in spec 06 — replaced with Tahoe squircle compliance language.
- All `AppIcon.appiconset/` references in `T-BRAND-ASSETS` and `T-REPO-INIT` — replaced with `AppIcon.icon` + the legacy fallback note.

**Provenance for the platform claims:** the Tahoe targeting decisions are based on Apple's published requirements current as of late 2025 / early 2026: `.icon` format introduced in Tahoe (Icon Composer in Xcode 26), Liquid Glass adoption pattern from WWDC 2025 sessions, the `UIDesignRequiresCompatibility` opt-out flag, and the App Store Connect April 2026 SDK 26 requirement. These are referenced rather than restated; future Apple changes will require an A19+ revision.
